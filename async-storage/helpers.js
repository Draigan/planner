...
// Set Storage
const storeAppState = async (value) => {
  try {
    await AsyncStorage.setItem("appstate", value);
  } catch (e) {
    error.log(e);
  }
};
...
// We load the initial values just once and use appState to check if its ran already
async function initialValues() {
  const appState = await getAppState();
  console.log(appState);
  if (!appState) {
    console.log("Runnign in if")
    storeDataDraigan({
      userName: "Draigan",
      slug: "draigan",
      tasks: [
